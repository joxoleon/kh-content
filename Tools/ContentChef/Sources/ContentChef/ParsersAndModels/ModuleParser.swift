...
class LearningModuleParser {
    ...
    func parseLearningModule(from fileURL: URL) throws -> Parsing.LearningModule {
    ...
    func parseModules(in directory: URL) throws -> [Parsing.LearningModule] {
        ...
        var modules: [Parsing.LearningModule] = []
    ...
    private func decodeYAML(_ yamlContent: String) throws -> Parsing.LearningModule? {
        let decoder = YAMLDecoder()
        do {
            let learningModule = try decoder.decode(Parsing.LearningModule.self, from: yamlContent)
            return learningModule
        } catch {
